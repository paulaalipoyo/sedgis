...
function PovertyStats() {
    const [addModalshow, setAddModal] = React.useState(false);
    const [alert, setalert] = React.useState(false);
    const [povertystats, setPovertyStats] = useState([]);
    ...
    const getSubIndicators = () => {
        axios
            .get(
                `${REACT_APP_API_URL}/povertystats-sub-indicators.php/subindicators`
            )
            .then((response) => {
                const { data } = response;
                if (response.status === 200) {
    ...
    function getPovetyStats() {
        axios
            .get(`${REACT_APP_API_URL}/povertystats.php/povertystat`)
            .then(function (response) {
                console.log(response.data);
                setPovertyStats(response.data);
            });
    }
    ...
    const addHandleSubmit = () => {
        axios
            .post(
                `${REACT_APP_API_URL}/povertystats.php/povertystat/save`,
                inputs
            )
            .then(function (response) {
                console.log(response.data);
                getPovetyStats(response.data);
            });
    };
        ...
        let options = {
            ...
            message: (
                <div className="alert-text">
                    ...
                    <span data-notify="message">
                        Entry deleted from Poverty Statistics
                    </span>
                </div>
            ),
            type: type,
            ...
            <ReactBSAlert
                ...
                onConfirm={() => {
                    getPovetyStats(
                        axios
                            .delete(
                                `${REACT_APP_API_URL}/povertystats.php/${id}/delete`
                            )
                            .then(function (response) {
                                console.log(response.data);
                                getPovetyStats(response.data);
    ...
    const onAfterSaveCell = (id) => {
        axios
            .put(`${REACT_APP_API_URL}/povertystats.php/povertystat/${id}/edit`)
            .then(function (response) {
                console.log(response.data);
            });
